...
export function LeadCard({ lead, onClick, isOverlay }: LeadCardProps) {
  ...
  const isOverdue =
    lead.nextFollowUp &&
    isPast(new Date(lead.nextFollowUp)) &&
  ...
  // Get Fathom icon color based on pipeline
  const getFathomIconColor = () => {
  ...
  return (
    <div
      ...
      className={cn(
        ...
        isOverlay &&
          "opacity-100 shadow-xl ring-2 ring-primary rotate-2 scale-105 cursor-grabbing z-50 bg-card",
    ...
    >
      <div className="flex justify-between items-start mb-2">
        <div className="flex items-center gap-1.5 flex-1 min-w-0">
          {(lead.recordingLink || lead.fathomRecordingId) && (
            <span title="Has Fathom recording">
              <Video className={cn("h-3 w-3 shrink-0", getFathomIconColor())} />
            </span>
          )}
        ...
        {(lead.actionItems || []).length > 0 && (
          <div
            className="h-2 w-2 rounded-full bg-orange-500 animate-pulse shrink-0 ml-1"
            title="Action Needed"
          />
        )}
      </div>
