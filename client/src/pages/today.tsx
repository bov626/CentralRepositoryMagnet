...
export default function TodayPage() {
    ...
    const waitingOnMe = leads.filter(
        (l) =>
            (l.actionItems || []).length > 0 &&
            !isToday(new Date(l.nextFollowUp || "")),
    ); // Avoid duplicates if it's already in due today

    // Mock pitch calls for today (in reality this would filter by stage + date)
    const pitchCalls = leads.filter(
        (l) =>
            l.stage === "pitch-call" &&
            (l.nextFollowUp ? isToday(new Date(l.nextFollowUp)) : true),
    );
